...
// Since we reset the file, we need to redo the changes properly without breaking syntax
...
// 2. Fix the state init in OverviewPage for layouts and heights
content = content.replace(/const \[layouts, setLayouts\] = useState<any>\(\{/, `const [layouts, setLayouts] = useState<any>({`);
...
const sidebarContentRegex = /\.eg-sidebar__content \{\s*display: flex;\s*flex: 1;\s*flex-direction: column;\s*gap: 0\.8rem;\s*min-height: 0;\s*overflow-y: auto;\s*padding-bottom: 0\.8rem;\s*-webkit-overflow-scrolling: touch;\s*overscroll-behavior: contain;\s*\}/;
...
const desktopSidebarContentRegex = /\.eg-sidebar__content \{\s*gap: 1rem;\s*overflow: visible;\s*padding-bottom: 0;\s*\}/;
