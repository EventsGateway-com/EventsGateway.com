...
content = content.replace(/type NavItem = \{\n  label: string;\n  href: string;\n\};/, `type NavItem = {
  label: string;
  href: string;
  icon?: React.ReactNode;
};`);

// 2. Define icons and update getNavGroups
const oldGetNavGroupsRegex = /function getNavGroups\(isAdmin: boolean\): NavGroup\[\] \{[\s\S]*?(?=export function SiteContextParams\(\))/;
...
content = content.replace(oldGetNavGroupsRegex, newGetNavGroups);

// 3. Update the NavLink render inside App()
const navLinkRenderRegex = /<NavLink\s*className=\{\(\{ isActive \}\) => \`eg-nav-link\$\{isActive \? " is-active" : ""\}\`\}\s*key=\{item\.href\}\s*to=\{item\.href\}\s*>\s*\{item\.label\}\s*<\/NavLink>/g;

const newNavLinkRender = `<NavLink
...
content = content.replace(navLinkRenderRegex, newNavLinkRender);

fs.writeFileSync(appTsxPath, content);
console.log("Updated getNavGroups with icons");
